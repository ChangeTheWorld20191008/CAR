...
def validation(img, name, save_imgs=False, save_dir=None):
    ...
    print(f"[TMP]: downscaled_img type is {type(downscaled_img)}, size is {downscaled_img.size()}")
    print(f"[TMP]: {downscaled_img}")
    ...
    if save_imgs and save_dir:
        ...
        img.save(os.path.join(save_dir, name + '_recon.jpg'))
...
if __name__ == '__main__':
    img_list = glob(os.path.join(args.img_dir, '**', '*.jpg'), recursive=True)
